...
- [x] Raw-range Save's `.quarter`/`.quarterIndex` NaN bug — fixed.
  **Data audit complete (2026-08-03)**: queried `dhor_log` for any row
  with a non-integer, non-positive, or reversed `segment_from`/
  `segment_to` — zero matches. Since the bug could only ever produce
  `NaN` (never a plausible-but-wrong value), this means saving a
  raw-range entry while the bug was live most likely failed outright
  rather than writing bad data. No repair needed; existing history is
  unaffected.
