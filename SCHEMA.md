...
| `haidh_period_length` | INTEGER | Added in migration 0011. Same purpose as above. Labeled "How many haidh days per cycle" in Setup as of V3.39. V4.2.14 caps this globally at 10 official days for every supported ruling/profile (`shared/haidhRules.js`'s `haidhOfficialMaxDuration`); the existing 11-touched-date code allowance is only the partial-day edge. |
...
| `haidh_ruling` | TEXT | Added in migration 0018 (V3.39). `hanafi` / `shafii`, `NOT NULL DEFAULT 'hanafi'`. Retained for profile/UI compatibility. As of V4.2.14 the Haidh maximum is globally 10 official days regardless of this field; it no longer increases the duration cap. Defaults silently rather than blocking. |
...
| `student_id` | TEXT (FK) | → `students.id`. |
| `date` | TEXT | `YYYY-MM-DD`. |
| `status` | TEXT | `present` / `absent` / `haidh` / `predicted-haidh` (and `predicted-absent` since 0029). Auto-set to `present` by the personal-log attendance sync; a Maktab log also wins over an exact Haidh mark and Maktab attendance itself is derived from the actual Maktab log tables. V3.39: setting `haidh`/`predicted-haidh` (`handleSetAttendance`, `worker/src/attendance.js`) is capped by the student's ruling duration and the 15-official/14-code-day gap via `shared/haidhRules.js`. **V4.2.14 has no probable Haidh state.** Raw confirmed/predicted rows are normalized against Maktab activity plus explicit stop evidence by `worker/src/haidhTimeline.js`; activity wins on its date and terminates the old episode so stale stored rows cannot resume it. A confirmed mark resets/rebuilds predictions from the first confirmed day. Never deletes log rows. **V4.2.11:** a teacher/admin write whose final status is confirmed `haidh` also promotes that student's existing profile to `gender='F', track_haidh=1`; `predicted-haidh` does not. |
