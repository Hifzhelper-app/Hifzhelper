...
1. **Item 68 — mobile Maktab Summary values stop collapsing.** The three log lines are now a real `96px + 1fr` grid instead of the V4.2.2 flex line that let the value fall to min-content width. `Juz 28 H1`, `Juz 4 Q2`, short verse ranges and similar values now use the full right-hand column and wrap only when genuinely necessary. The value and its `+N` badge are wrapped as one grid item so the badge cannot auto-flow onto a stray second row.
   **Device-review correction:** the older desktop `td:nth-child(...)` widths were still winning on phones because they are more specific than the initial mobile `td { width:100% }` reset. That left the Student cell at 21% and each log cell at 24%, exactly matching the narrow-content/large-empty-space symptom seen on device. Mobile now overrides those widths at matching specificity so every stacked cell consumes the full card width; the equal-width name-pill rule itself is unchanged.
2. **Same-width student-name pills (user refinement during the build).** Every Maktab Summary name pill fills the same Student-column width. Long names are trialled with single-line ellipsis instead of stretching the pill; the full name remains in the DOM and `title`. On mobile the name cell reserves the attendance icon's top-right corner so the full-width pill cannot run underneath it.
...
**Versioning:** this is a served frontend release, so all asset `?v=` tags, `CACHE_NAME`, the service-worker precache list and every css/js/shared build header move together to **4.2.8**.
