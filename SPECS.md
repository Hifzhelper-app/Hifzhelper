# Hifzhelper — Delivered specs archive

The build spec and the reasoning behind every **shipped** delivery, moved
out of `TODO.md` on 2026-08-17 (V3.67.3) so that file could go back to
being a list of things that are not done.

**This file is history. Nothing in it is an outstanding action.**
Instructions inside these entries — "run the migration first", "deploy the
worker before the frontend", "awaiting start building" — were true on the
day they were written and are preserved as a record of what that delivery
required. For current migration and deploy state, see the MIGRATION STATUS
block at the top of `TODO.md`. For what changed and which files were
touched, see `CHANGELOG.md`. This file answers a third question those two
do not: *why it was built that way, and what was rejected*.
...
- **V3.64.1** — V3.64.0 audited against the three-inputs rule — two real defects fixed BEFORE upload
- **V3.64.0** — the maktab day view IS the PJ day view — Option A, reuse instead of copy
- **V3.63.0** — date pill on the PJ's own grid + haidh as a yellow toggle, no banner — device feedback
- **V3.62.0** — maktab day view rebuilt as a real COPY of the PJ day view — device feedback
- **V3.61.1** — summary header/column alignment + date pill — device feedback on V3.61.0
- **V3.61.0** — (e2) UI round from device screenshots — built to the spec below
- **V3.59.1** — (e1) response-shape crash fix — built to the spec below, shipped inside the V3.60.0 zip
- Done — Maktab delivery (e) — SPLIT CONFIRMED, both halves built. (e1): V3.59.0, 2026-08-15. (e2): V3.60.0, 2026-08-16 (see its own
- **V3.60.0** — Maktab delivery (e2) — teacher day view + prepop + write path — built to the (e) spec below + the fifth/sixth/
- **V3.58.0** — Maktab delivery (d) — worker endpoints for the three maktab logs — built to the spec below. MAKTAB DEPLOYMENT 
- **V3.57.0** — Maktab delivery (c) — migration 0019, three maktab_* tables — built to the spec below. MIGRATION RUN on hifzhe
- **V3.56.0** — lost-note bug fix + Maktab delivery (b) PJ notes private-by-default — built to the spec below
- **V3.55.0** — Maktab delivery (a) — `isTeacherOrAbove` refactor — built to the spec below
- **V3.54.0** — attendance stays in sync with edited/deleted log dates — built to the spec below
- **V3.53.2** — lap list moved down + taller (fits ~10 rows) — built to the spec below
- **V3.53.1** — maximised-timer layout fixes from a device screenshot — built to the spec below
- **V3.53.0** — Journal→Summary rename + timer wake lock + full lap list — built to the spec below
- **V3.52.0** — Tadabbur edit parity — the popup editor, built to the spec below
- **V3.51.2** — Tadabbur save 500 + two companion regressions — built to the spec below
- **V3.51.1** — Edit-popup desktop fixes + heading/button polish — built to the spec below
- **V3.51.1** — edit bottombar styling tweaks — built 2026-08-15. [Heading corrected 2026-08-17: it read "awaiting start build
- **V3.51.0** — Edit screens redesigned as popups, editable dates/portions, new confirm flow — built to the spec below
- **V3.50.2** — Tadabbur header rework + Dhor/Sabaq UI set — built to the spec below
- **V3.50.1** — BUG, log detail cards' date selector not opening on iOS — fixed to the spec below
- **V3.50.0** — Confirm-selection checkboxes restyled to the Sabaq Dhor pattern — built to the spec below
- **V3.49.0** — Juz Tracker "Free play" mode — built to the spec below
- **V3.48.0** — SIH Eraser — built to the spec below
- **V3.47.0** — SIH vector-region engine ("Option C") — built to the spec below
- **V3.46.0** — "Surahs in my Heart" (SIH) — built to the spec below, agreed 2026-08-11
...
## Done — Maktab delivery (e) — SPLIT CONFIRMED, both halves built. (e1): V3.59.0, 2026-08-15. (e2): V3.60.0, 2026-08-16 (see its own entry). [Heading corrected 2026-08-17: it still read "(e2): OPEN … awaiting start building" three deliveries after (e2) shipped.]
...
## Done — V3.57.0 (2026-08-15): Maktab delivery (c) — migration 0019, three maktab_* tables — built to the spec below. MIGRATION RUN on hifzhelper-maktab1 (ONLY), confirmed 2026-08-17
...
First of the six maktab deliveries (see the design entry above).
Auth/permission change with LIVE blast radius, deliberately its own
delivery. Worker-only.

**What:** `auth.role === 'teacher'` / `!== 'teacher'` currently gates
12 sites across 7 worker files; an admin fails every one of them
today. Agreed rule: admin counts as teacher EVERYWHERE — roles are a
strict hierarchy (student < teacher < admin), not independent flags.

**How:** one helper in `worker/src/utils.js`:
```
export function isTeacherOrAbove(auth) {
  return !!auth && (auth.role === 'teacher' || auth.role === 'admin');
}
```
Every one of the 12 sites switched to it (`!isTeacherOrAbove(auth)`
where the check was `!== 'teacher'`; `isTeacherOrAbove(auth)` where
it was `=== 'teacher'`). Enumerated against the actual sandbox, not
memory: attendance.js ×2 (lines 10, 36), dhorLog.js ×2 (32, 51),
plans.js ×1 (21), position.js ×1 (18), reflections.js ×2 (21, 35),
sabaqDhorLog.js ×2 (20, 36), sabaqLog.js ×2 (24, 42). plans.js and
position.js have no utils.js import today — both gain one; the other
five extend their existing import line.

**Deliberately NOT changed:**
- `admin.js`'s `requireAdmin` (role === 'admin' — admin-only screens
  stay admin-only, hierarchy runs one way) and its line-52 role-value
  validator (`['student','teacher','admin'].includes(body.role)` — a
  data check, not a permission gate).
- `js/auth.js` line 50 (`role === 'admin'` → admin nav item) — same
  reason; a frontend gate for an admin-only screen. Refactor is
  worker-only as a result.

**Behaviour change, stated plainly:** after this, an admin logging in
can do everything a teacher can against the existing app — save into
another student's PJ, read their logs/plans/position/attendance. Not
a no-op. Low risk today (ADMIN-01 bootstrap account, no real users)
but real.

**Verification:** node:sqlite simulation driving the actual handlers
with three auth objects (student, teacher, admin) against another
student's id: student → 403 everywhere it should; teacher → allowed;
admin → NOW allowed, identical to teacher. Plus the helper itself
(null/undefined auth, unknown role → false).

**Built + verified:** built exactly as spec'd — helper in utils.js,
all 12 sites swapped (verified by grep: zero literal teacher-role
checks remain outside the helper's own body; exactly 12 helper uses
+ 1 definition), 5 import lines extended, plans.js/position.js gained
a fresh import. Every touched module confirmed to import cleanly
(would have caught a missing import that only shows at runtime).
Behavioural harness against the real handlers: 45/45 — all 7 GET
gates and all 5 POST gates (the 4 logs + attendance) exercised with
each of the three roles; the POST side checks WHICH student_id the
row actually lands on (student's foreign student_id silently ignored
→ own id; teacher's honoured; admin's NOW honoured); a student's own-
data access confirmed unchanged. V3.54.0's 22-check attendance-sync
harness (same worker files) re-run green, no regression. 96 total.
...
## Done — V3.51.1: edit bottombar styling tweaks — built 2026-08-15. [Heading corrected 2026-08-17: it read "awaiting start building" while the body below it recorded all ten items as built and verified.]
