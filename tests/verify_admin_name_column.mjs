...
try {
  ...
  check('desktop headers place Unique ID after Haidh', [...header.querySelectorAll('th')].map(x=>x.textContent).join(',')==='Name,WhatsApp,Role,Group,Status,Haidh,Unique ID,Actions');
  ...
  check('ID follows Haidh in the user row', row.cells[5].dataset.label==='Haidh Settings' && row.cells[6].dataset.label==='Unique ID' && row.cells[6].textContent==='A');
  ...
  check('desktop registration matches Name-first and ID-after-Haidh order', w.document.querySelector('.admin-row-new').cells[0].querySelector('#admin_new_name')!==null && w.document.querySelector('.admin-row-new').cells[6].dataset.label==='Unique ID');
