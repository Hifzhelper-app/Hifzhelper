...
const v42111Page = (html.match(/js\/app\.js\?v=([0-9.]+)/) || [])[1];
...
check('V4.2.11.1 pins still identify untouched files while later-edited files carry their current last-edit headers',
  /^\/\* Hifzhelper build 4\.2\.15\.1 \| css\/admin\.css \*\//.test(css)
  && /^\/\* Hifzhelper build 4\.2\.15\.1 \| css\/detail-pages\.css \*\//.test(detailCss)
  && /^\/\* Hifzhelper build 4\.2\.15\.1 \| js\/adminPage\.js \*\//.test(admin)
  && /^\/\* Hifzhelper build 4\.2\.14 \| js\/haidhDetailScreen\.js \*\//.test(haidh)
  && /^\/\* Hifzhelper build 4\.2\.15\.1 \| js\/maktabAttendancePage\.js \*\//.test(register)
