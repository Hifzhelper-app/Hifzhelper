...
check('page/cache keys have advanced together beyond the V4.2.11.1 patch',
  /js\/app\.js\?v=4\.2\.11\.2/.test(html) && /CACHE_NAME = 'hifzhelper-v4\.2\.11\.2'/.test(sw));
check('served files actually edited by this patch carry 4.2.11.1 headers',
  /^\/\* Hifzhelper build 4\.2\.11\.1 \| css\/admin\.css \*\//.test(css)
  && /^\/\* Hifzhelper build 4\.2\.11\.2 \| css\/detail-pages\.css \*\//.test(detailCss)
  && /^\/\* Hifzhelper build 4\.2\.11\.1 \| js\/adminPage\.js \*\//.test(admin)
  && /^\/\* Hifzhelper build 4\.2\.11\.2 \| js\/haidhDetailScreen\.js \*\//.test(haidh)
  && /^\/\* Hifzhelper build 4\.2\.11\.2 \| js\/maktabAttendancePage\.js \*\//.test(register)
  && /^\/\* Hifzhelper build 4\.2\.11\.2 \| js\/sw\.js \*\//.test(sw));
