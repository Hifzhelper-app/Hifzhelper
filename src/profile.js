/* Hifzhelper build 4.2.15.7 | worker/src/profile.js */
...
export async function handleGetProfile(request, env, auth) {
  ...
  // V4.2.15.7: the same boot-time settings read also carries the Hifz
  // class Zoom link. Students must be able to see the journal Zoom action
  // without widening /maktab/settings beyond its existing teacher+ read gate.
  // A migration-order fallback preserves the timezone if an older DB is
  // briefly serving a newer Worker before 0030 has been applied.
  try {
    const ms = await env.DB.prepare('SELECT timezone, zoom_link FROM maktab_settings WHERE id = 1').first();
    row.maktab_timezone = ms && ms.timezone ? ms.timezone : null;
    row.maktab_zoom_link = ms && ms.zoom_link ? ms.zoom_link : null;
  } catch (e) {
    try {
      const ms = await env.DB.prepare('SELECT timezone FROM maktab_settings WHERE id = 1').first();
      row.maktab_timezone = ms && ms.timezone ? ms.timezone : null;
    } catch (e2) { row.maktab_timezone = null; }
    row.maktab_zoom_link = null;
  }
